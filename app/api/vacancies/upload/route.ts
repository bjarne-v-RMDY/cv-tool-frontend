...
const CONTAINER_NAME = 'vacancy-files'
const QUEUE_NAME = 'vacancy-processing-queue'
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        try {
          ...
          console.log(`Added to vacancy processing queue: ${file.name}`)
          ...
          await fetch(`${process.env.next_public_base_url || 'http://localhost:3000'}/api/activity-log`, {
            ...
            body: JSON.stringify({
              type: 'processing',
              title: 'Vacancy File Queued for Processing',
    ...
    console.error('Vacancy upload API error:', error)
    return NextResponse.json(
